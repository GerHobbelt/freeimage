...
static WebPMuxError CreateFrameFragmentData(
  ...
  if (frame_frgm_bytes == nullptr) return WEBP_MUX_MEMORY_ERROR;
...
static WebPMuxError GetImageData(const WebPData* const bitstream,
  ...
  if (bitstream->size < TAG_SIZE ||
    ...
    *image = *bitstream;
    ...
    wpi = mux->images_;
    assert(wpi != nullptr && wpi->img_ != nullptr);
    *image = wpi->img_->data_;
    if (wpi->alpha_ != nullptr) {
...
WebPMuxError WebPMuxSetChunk(WebPMux* mux, const char fourcc[4],
  ...
  uint32_t tag;
  WebPMuxError err;
  if (mux == nullptr || fourcc == nullptr || chunk_data == nullptr ||
      chunk_data->bytes == nullptr || chunk_data->size > MAX_CHUNK_PAYLOAD) {
...
WebPMuxError WebPMuxSetImage(WebPMux* mux, const WebPData* bitstream,
  ...
  // Sanity checks.
  if (mux == nullptr || bitstream == nullptr || bitstream->bytes == nullptr ||
      bitstream->size > MAX_CHUNK_PAYLOAD) {
    return WEBP_MUX_INVALID_ARGUMENT;
  }

  if (mux->images_ != nullptr) {
...
WebPMuxError WebPMuxPushFrame(WebPMux* mux, const WebPMuxFrameInfo* frame,
  ...
  // Sanity checks.
  if (mux == nullptr || frame == nullptr) return WEBP_MUX_INVALID_ARGUMENT;
  ...
  if (bitstream->bytes == nullptr || bitstream->size > MAX_CHUNK_PAYLOAD) {
    return WEBP_MUX_INVALID_ARGUMENT;
  }

  if (mux->images_ != nullptr) {
    const WebPMuxImage* const image = mux->images_;
    const uint32_t image_id = (image->header_ != nullptr) ?
  ...
  err = SetAlphaAndImageChunks(bitstream, copy_data, &wpi);
  if (err != WEBP_MUX_OK) goto Err;
  assert(wpi.img_ != nullptr);  // As SetAlphaAndImageChunks() was successful.
...
WebPMuxError WebPMuxSetAnimationParams(WebPMux* mux,
  ...
  if (mux == nullptr || params == nullptr) return WEBP_MUX_INVALID_ARGUMENT;
...
WebPMuxError WebPMuxDeleteChunk(WebPMux* mux, const char fourcc[4]) {
  if (mux == nullptr || fourcc == nullptr) return WEBP_MUX_INVALID_ARGUMENT;
  return MuxDeleteAllNamedData(mux, ChunkGetTagFromFourCC(fourcc));
}

WebPMuxError WebPMuxDeleteFrame(WebPMux* mux, uint32_t nth) {
  if (mux == nullptr) return WEBP_MUX_INVALID_ARGUMENT;
...
static WebPMuxError GetFrameFragmentInfo(
  ...
  assert(frame_frgm_chunk != nullptr);
...
static WebPMuxError GetImageInfo(const WebPMuxImage* const wpi,
  ...
  if (width != nullptr) *width = wpi->width_;
  if (height != nullptr) *height = wpi->height_;
  return WEBP_MUX_OK;
}
...
static WebPMuxError CreateVP8XChunk(WebPMux* const mux) {
  ...
  const WebPMuxImage* images = nullptr;

  assert(mux != nullptr);
  images = mux->images_;  // First image.
  if (images == nullptr || images->img_ == nullptr ||
      images->img_->data_.bytes == nullptr) {
    return WEBP_MUX_INVALID_ARGUMENT;
  }
  ...
  // Set flags.
  if (mux->iccp_ != nullptr && mux->iccp_->data_.bytes != nullptr) {
    flags |= ICCP_FLAG;
  }
  if (mux->exif_ != nullptr && mux->exif_->data_.bytes != nullptr) {
    flags |= EXIF_FLAG;
  }
  if (mux->xmp_ != nullptr && mux->xmp_->data_.bytes != nullptr) {
    flags |= XMP_FLAG;
  }
  if (images->header_ != nullptr) {
  ...
  if (flags == 0 && mux->unknown_ == nullptr) {
...
static WebPMuxError MuxCleanup(WebPMux* const mux) {
  ...
  if (num_frames == 1 || num_fragments == 1) {
    ...
    assert(frame_frag != nullptr);
    if (frame_frag->header_ != nullptr &&
...
WebPMuxError WebPMuxAssemble(WebPMux* mux, WebPData* assembled_data) {
  ...
  if (data == nullptr) return WEBP_MUX_MEMORY_ERROR;
