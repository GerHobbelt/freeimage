...
WEBP_DSP_INIT_FUNC(WebPInitUpsamplers) {
  ...
  assert(WebPUpsamplers[MODE_RGBA] != nullptr);
  assert(WebPUpsamplers[MODE_BGRA] != nullptr);
  assert(WebPUpsamplers[MODE_rgbA] != nullptr);
  assert(WebPUpsamplers[MODE_bgrA] != nullptr);
#if !defined(WEBP_REDUCE_CSP) || !WEBP_NEON_OMIT_C_CODE
  assert(WebPUpsamplers[MODE_RGB] != nullptr);
  assert(WebPUpsamplers[MODE_BGR] != nullptr);
  assert(WebPUpsamplers[MODE_ARGB] != nullptr);
  assert(WebPUpsamplers[MODE_RGBA_4444] != nullptr);
  assert(WebPUpsamplers[MODE_RGB_565] != nullptr);
  assert(WebPUpsamplers[MODE_Argb] != nullptr);
  assert(WebPUpsamplers[MODE_rgbA_4444] != nullptr);
#endif
