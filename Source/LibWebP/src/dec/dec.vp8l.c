  ...
  while (src < src_last) {
    ...
    AdvanceByOne:
      ...
      if (col >= width) {
        col = 0;
        ++row;
        if ((row % NUM_ARGB_CACHE_ROWS == 0) && (process_func != nullptr)) {
...
int VP8LDecodeAlphaHeader(ALPHDecoder* const alph_dec,
  ...
  if (alph_dec->vp8l_dec_ == nullptr) return 0;
  ...
  VP8InitIo(io);
  WebPInitCustomIo(nullptr, io);  // Just a sanity Init. io won't be used.
  io->opaque = output;
