...
void _TIFFsetLong8Array(uint64** lpp, uint64* lp, uint32_t n)
	...
	default: {
		...
		else
		{
			...
			if (fip->field_tag == TIFFTAG_DOTRANGE 
				...
				uint16_t v[2];
				v[0] = (uint16_t)va_arg(ap, int);
				v[1] = (uint16_t)va_arg(ap, int);
...
TIFFAdvanceDirectory(TIFF* tif, uint64* nextdir, uint64* off)
{
	...
	if (isMapped(tif))
	{
		...
		if (!(tif->tif_flags&TIFF_BIGTIFF))
		{
			...
			if (((uint64)poffa!=poff)||(poffb<poffa)||(poffb<(tmsize_t)sizeof(uint16_t))||(poffb>tif->tif_size))
			{
