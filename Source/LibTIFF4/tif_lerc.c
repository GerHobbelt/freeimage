...
static int LERCPreDecode(TIFF *tif, uint16_t s)
{
    ...
    lerc_ret =
        lerc_getBlobInfo(lerc_data, lerc_data_size, infoArray, nullptr, 8, 0);
    if (lerc_ret != 0)
    {
    ...
    lerc_ret = lerc_decode(lerc_data, lerc_data_size,
                           ...
                           use_mask ? 1 : 0,
#endif
                           use_mask ? sp->mask_buffer : nullptr, ndims,
...
static int LERCPostEncode(TIFF *tif)
{
             ...
             (td->td_bitspersample == 32 || td->td_bitspersample == 64))
    {
        ...
        lerc_ret = lerc_computeCompressedSize(
            ...
            sp->segment_height,
            1,
            use_mask ? sp->mask_buffer : nullptr,
            sp->maxzerror,
            &numBytes);
        if( lerc_ret != 0 )
        {
    ...
    lerc_ret = lerc_encodeForVersion(
        ...
        use_mask ? 1 : 0,
#endif
        use_mask ? sp->mask_buffer : nullptr, sp->maxzerror, sp->compressed_buffer,
