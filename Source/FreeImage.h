...
typedef struct FIBITMAPINFOHEADER {
	uint32_t biSize;
	int32_t  biWidth;
	int32_t  biHeight;
	uint16_t biPlanes;
...
} FIBITMAPINFOHEADER, * PFIBITMAPINFOHEADER;

typedef struct FIBITMAPINFO {
	FIBITMAPINFOHEADER bmiHeader;
	FIRGBA8            bmiColors[1];
} FIBITMAPINFO, * PFIBITMAPINFO;



// Indexes for byte arrays, masks and shifts for treating pixels as words ---
// These coincide with the order of FIRGBA8 and FIRGB8 -------------------
...
FI_ENUM(FREE_IMAGE_MDTYPE) {
	...
	FIDT_PALETTE	= 14,	//! 32-bit FIRGBA8 
...
FI_STRUCT (Plugin) {
	FI_FormatProc format_proc FI_DEFAULT(nullptr);
	FI_DescriptionProc description_proc FI_DEFAULT(nullptr);
	FI_ExtensionListProc extension_proc FI_DEFAULT(nullptr);
	FI_RegExprProc regexpr_proc FI_DEFAULT(nullptr);
	FI_OpenProc open_proc FI_DEFAULT(nullptr);
	FI_CloseProc close_proc FI_DEFAULT(nullptr);
	FI_PageCountProc pagecount_proc FI_DEFAULT(nullptr);
	FI_PageCapabilityProc pagecapability_proc FI_DEFAULT(nullptr);
	FI_LoadProc load_proc FI_DEFAULT(nullptr);
	FI_SaveProc save_proc FI_DEFAULT(nullptr);
	FI_ValidateProc validate_proc FI_DEFAULT(nullptr);
	FI_MimeProc mime_proc FI_DEFAULT(nullptr);
	FI_SupportsExportBPPProc supports_export_bpp_proc FI_DEFAULT(nullptr);
	FI_SupportsExportTypeProc supports_export_type_proc FI_DEFAULT(nullptr);
	FI_SupportsICCProfilesProc supports_icc_profiles_proc FI_DEFAULT(nullptr);
	FI_SupportsNoPixelsProc supports_no_pixels_proc FI_DEFAULT(nullptr);
};
...
typedef void* (DLL_CALLCONV* FI_OpenProc2)(void* ctx, FreeImageIO* io, fi_handle handle, BOOL read);
...
typedef BOOL(DLL_CALLCONV* FI_SaveProc2)(void* ctx, FreeImageIO* io, FIBITMAP* dib, fi_handle handle, uint32_t page, uint32_t flags, void* data);
typedef BOOL(DLL_CALLCONV* FI_ValidateProc2)(void* ctx, FreeImageIO* io, fi_handle handle);
typedef const char* (DLL_CALLCONV* FI_MimeProc2)(void* ctx);
typedef BOOL(DLL_CALLCONV* FI_SupportsExportBPPProc2)(void* ctx, uint32_t bpp);
typedef BOOL(DLL_CALLCONV* FI_SupportsExportTypeProc2)(void* ctx, FREE_IMAGE_TYPE type);
typedef BOOL(DLL_CALLCONV* FI_SupportsICCProfilesProc2)(void* ctx);
typedef BOOL(DLL_CALLCONV* FI_SupportsNoPixelsProc2)(void* ctx);
typedef void(DLL_CALLCONV* FI_ReleaseProc2)(void* ctx);

FI_STRUCT(Plugin2) {
	FI_FormatProc2 format_proc FI_DEFAULT(nullptr);
	FI_DescriptionProc2 description_proc FI_DEFAULT(nullptr);
	FI_ExtensionListProc2 extension_proc FI_DEFAULT(nullptr);
	FI_RegExprProc2 regexpr_proc FI_DEFAULT(nullptr);
	FI_OpenProc2 open_proc FI_DEFAULT(nullptr);
	FI_CloseProc2 close_proc FI_DEFAULT(nullptr);
	FI_PageCountProc2 pagecount_proc FI_DEFAULT(nullptr);
	FI_PageCapabilityProc2 pagecapability_proc FI_DEFAULT(nullptr);
	FI_LoadProc2 load_proc FI_DEFAULT(nullptr);
	FI_SaveProc2 save_proc FI_DEFAULT(nullptr);
	FI_ValidateProc2 validate_proc FI_DEFAULT(nullptr);
	FI_MimeProc2 mime_proc FI_DEFAULT(nullptr);
	FI_SupportsExportBPPProc2 supports_export_bpp_proc FI_DEFAULT(nullptr);
	FI_SupportsExportTypeProc2 supports_export_type_proc FI_DEFAULT(nullptr);
	FI_SupportsICCProfilesProc2 supports_icc_profiles_proc FI_DEFAULT(nullptr);
	FI_SupportsNoPixelsProc2 supports_no_pixels_proc FI_DEFAULT(nullptr);
	FI_ReleaseProc2 release_proc FI_DEFAULT(nullptr);
};

// Plugin behaviour hould be invariant to FIF_SOMETHING enum value
typedef BOOL (DLL_CALLCONV* FI_InitProc2)(Plugin2* plugin, void* ctx);
...
FI_STRUCT (FIDEPENDENCY) {
	const char* name		FI_DEFAULT(nullptr);
	const char* fullVersion FI_DEFAULT(nullptr);	// Might include more components than major and minor digits, depends on each library style
...
#define FI_COLOR_IS_RGB_COLOR			0x00	//! FIRGBA8 color is a RGB color (contains no valid alpha channel)
#define FI_COLOR_IS_RGBA_COLOR			0x01	//! FIRGBA8 color is a RGBA color (contains a valid alpha channel)
 ...
 * If 'proc_address' is nullptr, then only existing plugin is unregistered.
 */
DLL_API BOOL DLL_CALLCONV FreeImage_ResetLocalPlugin2(FREE_IMAGE_FORMAT fif, FI_InitProc2 proc_address, void* ctx FI_DEFAULT(0), BOOL force FI_DEFAULT(FALSE));
...
DLL_API BOOL DLL_CALLCONV FreeImage_GetPixelColor(FIBITMAP *dib, unsigned x, unsigned y, FIRGBA8 *value);
DLL_API BOOL DLL_CALLCONV FreeImage_SetPixelIndex(FIBITMAP *dib, unsigned x, unsigned y, uint8_t *value);
DLL_API BOOL DLL_CALLCONV FreeImage_SetPixelColor(FIBITMAP *dib, unsigned x, unsigned y, FIRGBA8 *value);
 ...
 */
DLL_API FREE_IMAGE_COLOR_TYPE DLL_CALLCONV FreeImage_GetColorType2(FIBITMAP* dib, BOOL scan_alpha FI_DEFAULT(FALSE));
...
DLL_API BOOL DLL_CALLCONV FreeImage_GetBackgroundColor(FIBITMAP *dib, FIRGBA8 *bkcolor);
DLL_API BOOL DLL_CALLCONV FreeImage_SetBackgroundColor(FIBITMAP *dib, FIRGBA8 *bkcolor);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To4(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine1To16_555(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine4To16_555(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To16_555(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine1To16_565(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine4To16_565(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To16_565(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine1To24(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine4To24(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To24(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette);
...
DLL_API void DLL_CALLCONV FreeImage_ConvertLine8To32MapTransparency(uint8_t *target, uint8_t *source, int width_in_pixels, FIRGBA8 *palette, uint8_t *table, int transparent_pixels);
...
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_ColorQuantizeEx(FIBITMAP *dib, FREE_IMAGE_QUANTIZE quantize FI_DEFAULT(FIQ_WUQUANT), int PaletteSize FI_DEFAULT(256), int ReserveSize FI_DEFAULT(0), FIRGBA8 *ReservePalette FI_DEFAULT(nullptr));
 ...
 */
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_ConvertToFloat(FIBITMAP *dib, BOOL scale_linear FI_DEFAULT(TRUE));
 ...
 */
DLL_API BOOL DLL_CALLCONV FreeImage_MakeHistogram(FIBITMAP* dib, uint32_t binsNumber, void* minVal, void* maxVal, uint32_t* histR, uint32_t strideR FI_DEFAULT(1u),
	uint32_t* histG FI_DEFAULT(nullptr), uint32_t strideG  FI_DEFAULT(1u), uint32_t* histB FI_DEFAULT(nullptr), uint32_t strideB  FI_DEFAULT(1u), uint32_t* histL FI_DEFAULT(nullptr), uint32_t strideL FI_DEFAULT(1u));
...
DLL_API unsigned DLL_CALLCONV FreeImage_ApplyColorMapping(FIBITMAP *dib, FIRGBA8 *srccolors, FIRGBA8 *dstcolors, unsigned count, BOOL ignore_alpha, BOOL swap);
DLL_API unsigned DLL_CALLCONV FreeImage_SwapColors(FIBITMAP *dib, FIRGBA8 *color_a, FIRGBA8 *color_b, BOOL ignore_alpha);
...
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_Composite(FIBITMAP *fg, BOOL useFileBkg FI_DEFAULT(FALSE), FIRGBA8 *appBkColor FI_DEFAULT(nullptr), FIBITMAP *bg FI_DEFAULT(nullptr));
/**
 ...
 */
DLL_API BOOL DLL_CALLCONV FreeImage_DrawBitmap(FIBITMAP* dst, FIBITMAP* src, FREE_IMAGE_ALPHA_OPERATION alpha, int32_t left FI_DEFAULT(0), int32_t top FI_DEFAULT(0));
...
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_AllocateEx(int width, int height, int bpp, const FIRGBA8 *color, int options FI_DEFAULT(0), const FIRGBA8 *palette FI_DEFAULT(nullptr), unsigned red_mask FI_DEFAULT(0), unsigned green_mask FI_DEFAULT(0), unsigned blue_mask FI_DEFAULT(0));
DLL_API FIBITMAP *DLL_CALLCONV FreeImage_AllocateExT(FREE_IMAGE_TYPE type, int width, int height, int bpp, const void *color, int options FI_DEFAULT(0), const FIRGBA8 *palette FI_DEFAULT(nullptr), unsigned red_mask FI_DEFAULT(0), unsigned green_mask FI_DEFAULT(0), unsigned blue_mask FI_DEFAULT(0));
 ...
 */
DLL_API BOOL DLL_CALLCONV FreeImage_FindMinMax(FIBITMAP* dib, double* min_brightness, double* max_brightness, void** min_ptr FI_DEFAULT(nullptr), void** max_ptr FI_DEFAULT(nullptr));
/**
 * Finds min and max value for every channel.
 * Return using same data type as stored image.
 */
DLL_API BOOL DLL_CALLCONV FreeImage_FindMinMaxValue(FIBITMAP* dib, void* min_value, void* max_value);

/**
 * Sets all image pixels to 'value'. The value must be of valid type.
 */
DLL_API BOOL DLL_CALLCONV FreeImage_Fill(FIBITMAP* dib, const void* value_ptr, size_t value_size);
 ...
 */
DLL_API BOOL DLL_CALLCONV CastPixelValue(FREE_IMAGE_TYPE src_type, const void* src_pixel, FREE_IMAGE_TYPE dst_type, void* dst_pixel);
