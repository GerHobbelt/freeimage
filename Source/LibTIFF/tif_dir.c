...
void _TIFFsetNString(char** cpp, char* cp, uint32_t n)
 ...
 */
static int
setExtraSamples(TIFFDirectory* td, va_list ap, uint32_t* v)
{
...
_TIFFVSetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_SUBFILETYPE:
		td->td_subfiletype = va_arg(ap, uint32_t);
		break;
	case TIFFTAG_IMAGEWIDTH:
		td->td_imagewidth = va_arg(ap, uint32_t);
		break;
	case TIFFTAG_IMAGELENGTH:
		td->td_imagelength = va_arg(ap, uint32_t);
		break;
	case TIFFTAG_BITSPERSAMPLE:
		td->td_bitspersample = (uint16_t) va_arg(ap, int);
		/*
	...
	case TIFFTAG_PHOTOMETRIC:
		td->td_photometric = (uint16_t) va_arg(ap, int);
		break;
	case TIFFTAG_THRESHHOLDING:
		td->td_threshholding = (uint16_t) va_arg(ap, int);
		break;
	...
	case TIFFTAG_MINSAMPLEVALUE:
		td->td_minsamplevalue = (uint16_t) va_arg(ap, int);
		break;
	case TIFFTAG_MAXSAMPLEVALUE:
		td->td_maxsamplevalue = (uint16_t) va_arg(ap, int);
		break;
	...
	case TIFFTAG_PAGENUMBER:
		td->td_pagenumber[0] = (uint16_t) va_arg(ap, int);
		td->td_pagenumber[1] = (uint16_t) va_arg(ap, int);
		break;
	case TIFFTAG_HALFTONEHINTS:
		td->td_halftonehints[0] = (uint16_t) va_arg(ap, int);
		td->td_halftonehints[1] = (uint16_t) va_arg(ap, int);
		break;
	...
	case TIFFTAG_MATTEING:
		td->td_extrasamples = (uint16_t) (va_arg(ap, int) != 0);
	...
	case TIFFTAG_IMAGEDEPTH:
		td->td_imagedepth = va_arg(ap, uint32_t);
		break;
	case TIFFTAG_SUBIFD:
		if ((tif->tif_flags & TIFF_INSUBIFD) == 0) {
			td->td_nsubifd = (uint16_t) va_arg(ap, int);
			_TIFFsetLongArray(&td->td_subifd, va_arg(ap, uint32_t*),
			    (long) td->td_nsubifd);
		} else {
	...
	case TIFFTAG_YCBCRPOSITIONING:
		td->td_ycbcrpositioning = (uint16_t) va_arg(ap, int);
		break;
	case TIFFTAG_YCBCRSUBSAMPLING:
		td->td_ycbcrsubsampling[0] = (uint16_t) va_arg(ap, int);
		td->td_ycbcrsubsampling[1] = (uint16_t) va_arg(ap, int);
		break;
	case TIFFTAG_TRANSFERFUNCTION:
		v = (td->td_samplesperpixel - td->td_extrasamples) > 1 ? 3 : 1;
		for (i = 0; i < v; i++)
			_TIFFsetShortArray(&td->td_transferfunction[i],
			    va_arg(ap, uint16_t*), 1L<<td->td_bitspersample);
		break;
	...
	case TIFFTAG_INKNAMES:
	     ...
	     * compression schemes and codec-specific tags are blindly copied.
             */
            if(fip == nullptr || fip->field_bit != FIELD_CUSTOM) {
		TIFFErrorExt(tif->tif_clientdata, module,
...
_TIFFVGetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_HALFTONEHINTS:
            ...
            *va_arg(ap, uint16_t*) = td->td_halftonehints[1];
            break;
	case TIFFTAG_COLORMAP:
            *va_arg(ap, uint16_t**) = td->td_colormap[0];
            *va_arg(ap, uint16_t**) = td->td_colormap[1];
            *va_arg(ap, uint16_t**) = td->td_colormap[2];
            break;
	case TIFFTAG_STRIPOFFSETS:
	case TIFFTAG_TILEOFFSETS:
            *va_arg(ap, uint32_t**) = td->td_stripoffset;
            break;
	case TIFFTAG_STRIPBYTECOUNTS:
	case TIFFTAG_TILEBYTECOUNTS:
            *va_arg(ap, uint32_t**) = td->td_stripbytecount;
            break;
	case TIFFTAG_MATTEING:
            *va_arg(ap, uint16_t*) =
	...
	case TIFFTAG_SUBIFD:
            ...
            *va_arg(ap, uint32_t**) = td->td_subifd;
            break;
	case TIFFTAG_YCBCRPOSITIONING:
...
static int
TIFFAdvanceDirectory(TIFF* tif, uint32_t* nextdir, toff_t* off)
{
	...
	{
		toff_t poff=*nextdir;
		if (poff+sizeof(uint16_t) > tif->tif_size)
		{
			TIFFErrorExt(tif->tif_clientdata, module, "%s: Error fetching directory count",
			    tif->tif_name);
			return (0);
		}
		_TIFFmemcpy(&dircount, tif->tif_base+poff, sizeof (uint16_t));
		if (tif->tif_flags & TIFF_SWAB)
			TIFFSwabShort(&dircount);
		poff+=sizeof (uint16_t)+dircount*sizeof (TIFFDirEntry);
		if (off != nullptr)
			*off = poff;
		if (((toff_t) (poff+sizeof (uint32_t))) > tif->tif_size)
		{
			TIFFErrorExt(tif->tif_clientdata, module, "%s: Error fetching directory link",
			    tif->tif_name);
			return (0);
		}
		_TIFFmemcpy(nextdir, tif->tif_base+poff, sizeof (uint32_t));
		if (tif->tif_flags & TIFF_SWAB)
		...
		if (off != nullptr)
			*off = TIFFSeekFile(tif,
			...
			(void) TIFFSeekFile(tif,
			    dircount*sizeof (TIFFDirEntry), SEEK_CUR);
		if (!ReadOK(tif, nextdir, sizeof (uint32_t))) {
			TIFFErrorExt(tif->tif_clientdata, module, "%s: Error fetching directory link",
			    tif->tif_name);
			return (0);
		}
 ...
 */
int
TIFFSetSubDirectory(TIFF* tif, uint32_t diroff)
{
...
TIFFUnlinkDirectory(TIFF* tif, tdir_t dirn)
{
	 ...
	 */
	nextdir = tif->tif_header.tiff_diroff;
	off = sizeof (uint16_t) + sizeof (uint16_t);
	for (n = dirn-1; n > 0; n--) {
		if (nextdir == 0) {
	...
	if (tif->tif_flags & TIFF_SWAB)
		TIFFSwabLong(&nextdir);
	if (!WriteOK(tif, &nextdir, sizeof (uint32_t))) {
