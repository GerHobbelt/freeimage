...
LoadStandardIcon(FreeImageIO *io, fi_handle handle, int flags, BOOL header_only) {
	...
	std::unique_ptr<FIBITMAP, decltype(&FreeImage_Unload)> dib(FreeImage_AllocateHeader(header_only, width, height, bit_count), &FreeImage_Unload);
	...
	if (bmih.biBitCount <= 8) {
		...
		io->read_proc(FreeImage_GetPalette(dib.get()), CalculateUsedPaletteEntries(bit_count) * sizeof(FIRGBA8), 1, handle);
	...
	if (bit_count == 16) {
		for (int y = 0; y < height; y++) {
			uint16_t *pixel = (uint16_t *)FreeImage_GetScanLine(dib.get(), y);
	...
	if (((flags & ICO_MAKEALPHA) == ICO_MAKEALPHA) && (bit_count < 32)) {
		FIBITMAP *dib32 = FreeImage_ConvertTo32Bits(dib.get());
		dib.reset(dib32);
...
SaveStandardIcon(FreeImageIO *io, FIBITMAP *dib, fi_handle handle) {
	...
	// write the bits
	const int width			= bmih->biWidth;
	const int height			= bmih->biHeight;
	const unsigned bit_count	= bmih->biBitCount;
	const unsigned line		= CalculateLine(width, bit_count);
	const unsigned pitch		= CalculatePitch(line);
	const int size_xor		= height * pitch;
	const int size_and		= height * WidthBytes(width);
	...
	if (FreeImage_IsTransparent(dib)) {
		...
		if (bit_count == 32) {
			...
			switch (FreeImage_GetBPP(dib)) {
				...
				{
					for (int y = 0; y < height; y++) {
						auto *bits = (const uint8_t*)FreeImage_GetScanLine(dib, y);
						for (int x = 0; x < width; x++) {
							// get pixel at (x, y)
...
Save(FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		for (k = 0; k < icon_header->idCount; k++) {
			...
			vPages.emplace_back(icon_dib, &FreeImage_Unload);
		}
