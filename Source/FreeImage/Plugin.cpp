...
#ifdef _MSC_VER
#pragma warning(
    disable : 4786) // identifier was truncated to 'number' characters
#endif
...
PluginNode *PluginList::FindNodeFromFormat(const char *format) {
  for (map<int, PluginNode *>::iterator i = m_plugin_map.begin();
    ...
    const char *the_format = ((*i).second->m_format != NULL)
                                 ? (*i).second->m_format
...
PluginNode *PluginList::FindNodeFromMime(const char *mime) {
  for (map<int, PluginNode *>::iterator i = m_plugin_map.begin();
       i != m_plugin_map.end(); ++i) {
    const char *the_mime = ((*i).second->m_plugin->mime_proc != NULL)
    ...
    */
    ...
    if (s_plugins) {
      ...
      */
      ...
      s_plugins->AddNode(InitPNM, NULL, "PBM", "Portable Bitmap (ASCII)", "pbm",
                         "^P1");
      s_plugins->AddNode(InitPNM, NULL, "PBMRAW", "Portable Bitmap (RAW)",
                         "pbm", "^P4");
      // s_plugins->AddNode(InitPCD);
      // s_plugins->AddNode(InitPCX);
      s_plugins->AddNode(InitPNM, NULL, "PGM", "Portable Greymap (ASCII)",
                         "pgm", "^P2");
      s_plugins->AddNode(InitPNM, NULL, "PGMRAW", "Portable Greymap (RAW)",
                         "pgm", "^P5");
      s_plugins->AddNode(InitPNG);
      s_plugins->AddNode(InitPNM, NULL, "PPM", "Portable Pixelmap (ASCII)",
                         "ppm", "^P3");
      s_plugins->AddNode(InitPNM, NULL, "PPMRAW", "Portable Pixelmap (RAW)",
                         "ppm", "^P6");
...
FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_RegisterLocalPlugin(
  ...
  return s_plugins->AddNode(proc_address, NULL, format, description, extension,
                            regexpr);
}
...
const char *DLL_CALLCONV FreeImage_GetFormatFromFIF(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != NULL) {
    ...
    return (node != NULL) ? (node->m_format != NULL)
                                ? node->m_format
...
const char *DLL_CALLCONV FreeImage_GetFIFMimeType(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != NULL) {
    ...
    return (node != NULL) ? (node->m_plugin != NULL)
                                ? (node->m_plugin->mime_proc != NULL)
...
const char *DLL_CALLCONV FreeImage_GetFIFExtensionList(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != NULL) {
    ...
    return (node != NULL) ? (node->m_extension != NULL)
                                ? node->m_extension
                                : (node->m_plugin->extension_proc != NULL)
...
const char *DLL_CALLCONV FreeImage_GetFIFDescription(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != NULL) {
    ...
    return (node != NULL) ? (node->m_description != NULL)
                                ? node->m_description
                                : (node->m_plugin->description_proc != NULL)
...
const char *DLL_CALLCONV FreeImage_GetFIFRegExpr(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != NULL) {
    ...
    return (node != NULL) ? (node->m_regexpr != NULL)
                                ? node->m_regexpr
                                : (node->m_plugin->regexpr_proc != NULL)
...
BOOL DLL_CALLCONV FreeImage_FIFSupportsICCProfiles(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != NULL) {
    ...
    return (node != NULL) ? (node->m_plugin->supports_icc_profiles_proc != NULL)
...
BOOL DLL_CALLCONV FreeImage_FIFSupportsNoPixels(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != NULL) {
    ...
    return (node != NULL) ? (node->m_plugin->supports_no_pixels_proc != NULL)
...
BOOL DLL_CALLCONV FreeImage_Validate(FREE_IMAGE_FORMAT fif, FreeImageIO *io,
  ...
  if (s_plugins != NULL) {
    ...
    if (node) {
      ...
      validated = (node != NULL)
                      ? (node->m_enabled)
                            ? (node->m_plugin->validate_proc != NULL)
