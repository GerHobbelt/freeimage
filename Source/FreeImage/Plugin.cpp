...
static PluginList *s_plugins = nullptr;
...
PluginList::AddNode(FI_InitProc init_proc, void *instance, const char *format,
  ...
  if (init_proc != nullptr) {
    ...
    // note we have memset to 0, so all unset pointers should be nullptr)

    init_proc(plugin, (int)m_plugin_map.size());

    // get the format string (two possible ways)

    const char *the_format = nullptr;

    if (format != nullptr) {
      the_format = format;
    } else if (plugin->format_proc != nullptr) {
      the_format = plugin->format_proc();
    }

    // add the node if it wasn't there already

    if (the_format != nullptr) {
...
PluginNode *PluginList::FindNodeFromFormat(const char *format) {
  for (map<int, PluginNode *>::iterator i = m_plugin_map.begin();
       i != m_plugin_map.end(); ++i) {
    const char *the_format = ((*i).second->m_format != nullptr)
                                 ? (*i).second->m_format
                                 : (*i).second->m_plugin->format_proc();
...
PluginNode *PluginList::FindNodeFromMime(const char *mime) {
  for (map<int, PluginNode *>::iterator i = m_plugin_map.begin();
       i != m_plugin_map.end(); ++i) {
    const char *the_mime = ((*i).second->m_plugin->mime_proc != nullptr)
                               ? (*i).second->m_plugin->mime_proc()
                               : "";

    if ((*i).second->m_enabled) {
      if ((the_mime != nullptr) && (strcmp(the_mime, mime) == 0)) {
        return (*i).second;
      }
    }
  }
...
PluginNode *PluginList::FindNodeFromFIF(int node_id) {
...
int PluginList::Size() const { return (int)m_plugin_map.size(); }

BOOL PluginList::IsEmpty() const { return m_plugin_map.empty(); }

PluginList::~PluginList() {
  for (map<int, PluginNode *>::iterator i = m_plugin_map.begin();
       i != m_plugin_map.end(); ++i) {
#ifdef _WIN32
    if ((*i).second->m_instance != nullptr) {
...
PluginList *DLL_CALLCONV FreeImage_GetPluginList() { return s_plugins; }

// =====================================================================
// Plugin System Initialization
// =====================================================================

void DLL_CALLCONV FreeImage_Initialise(BOOL load_local_plugins_only) {
...
// s_plugins->AddNode(InitJXR);
      ...
      if (!load_local_plugins_only) {
        ...
        // store the current directory. then set the directory to the
          ...
          if ((find_handle = (long)_findfirst(buffer, &find_data)) != -1L) {
            do {
              ...
              strncat(buffer, find_data.name, MAX_PATH + 200);
							...
							//TODO:  use LoadLibrary as LoadLibraryW instate, with proper unicode file names. 
							HINSTANCE instance = LoadLibraryA(buffer); //! No Unicode Support for now !

              if (instance != nullptr) {
                FARPROC proc_address = GetProcAddress(instance, "_Init@8");

                if (proc_address != nullptr) {
                  s_plugins->AddNode((FI_InitProc)proc_address,
                                     (void *)instance);
                } else {
...
void *DLL_CALLCONV FreeImage_Open(PluginNode *node, FreeImageIO *io,
                                  fi_handle handle, BOOL open_for_reading) {
  if (node->m_plugin->open_proc != nullptr) {
...
void DLL_CALLCONV FreeImage_Close(PluginNode *node, FreeImageIO *io,
                                  fi_handle handle, void *data) {
  if (node->m_plugin->close_proc != nullptr) {
...
FIBITMAP *DLL_CALLCONV FreeImage_LoadFromHandle(FREE_IMAGE_FORMAT fif,
                                                FreeImageIO *io,
  ...
  if ((fif >= 0) && (fif < FreeImage_GetFIFCount())) {
    ...
    if (node != nullptr) {
      if (node->m_plugin->load_proc != nullptr) {
        void *data = FreeImage_Open(node, io, handle, TRUE);

        FIBITMAP *bitmap =
            node->m_plugin->load_proc(io, handle, -1, flags, data);
...
FIBITMAP *DLL_CALLCONV FreeImage_Load(FREE_IMAGE_FORMAT fif,
  ...
  if (handle) {
    FIBITMAP *bitmap =
        FreeImage_LoadFromHandle(fif, &io, (fi_handle)handle, flags);
    ...
    FreeImage_OutputMessageProc(
        (int)fif, "FreeImage_Load: failed to open file %s", filename);
  }

  return nullptr;
}

FIBITMAP *DLL_CALLCONV FreeImage_LoadU(FREE_IMAGE_FORMAT fif,
                                       const wchar_t *filename, int flags) {
  FreeImageIO io;
  SetDefaultIO(&io);
#ifdef _WIN32
  ...
  if (handle) {
    ...
    FreeImage_OutputMessageProc((int)fif,
                                "FreeImage_LoadU: failed to open input file");
  }
#endif
  return nullptr;
}

BOOL DLL_CALLCONV FreeImage_SaveToHandle(FREE_IMAGE_FORMAT fif, FIBITMAP *dib,
                                         FreeImageIO *io, fi_handle handle,
                                         int flags) {
  ...
  if ((fif >= 0) && (fif < FreeImage_GetFIFCount())) {
    ...
    if (node) {
      if (node->m_plugin->save_proc != nullptr) {
        void *data = FreeImage_Open(node, io, handle, FALSE);

        BOOL result =
            node->m_plugin->save_proc(io, dib, handle, -1, flags, data);
...
BOOL DLL_CALLCONV FreeImage_Save(FREE_IMAGE_FORMAT fif, FIBITMAP *dib,
  ...
  if (handle) {
    BOOL success =
        FreeImage_SaveToHandle(fif, dib, &io, (fi_handle)handle, flags);
    ...
    FreeImage_OutputMessageProc(
        (int)fif, "FreeImage_Save: failed to open file %s", filename);
  }

  return FALSE;
}

BOOL DLL_CALLCONV FreeImage_SaveU(FREE_IMAGE_FORMAT fif, FIBITMAP *dib,
                                  const wchar_t *filename, int flags) {
  FreeImageIO io;
  SetDefaultIO(&io);
#ifdef _WIN32
  ...
  if (handle) {
    ...
    FreeImage_OutputMessageProc((int)fif,
                                "FreeImage_SaveU: failed to open output file");
  }
#endif
  return FALSE;
}
...
FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_RegisterLocalPlugin(
    FI_InitProc proc_address, const char *format, const char *description,
    const char *extension, const char *regexpr) {
  return s_plugins->AddNode(proc_address, nullptr, format, description, extension,
                            regexpr);
}

#ifdef _WIN32
FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_RegisterExternalPlugin(
    const char *path, const char *format, const char *description,
    const char *extension, const char *regexpr) {
  if (path != nullptr) {
		//TODO:  use LoadLibrary as LoadLibraryW instate, with proper unicode file names. 
		HINSTANCE instance = LoadLibraryA(path); //! No Unicode Support for now !

    if (instance != nullptr) {
      FARPROC proc_address = GetProcAddress(instance, "_Init@8");

      FREE_IMAGE_FORMAT result =
...
int DLL_CALLCONV FreeImage_SetPluginEnabled(FREE_IMAGE_FORMAT fif,
                                            BOOL enable) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);
...
int DLL_CALLCONV FreeImage_IsPluginEnabled(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? node->m_enabled : FALSE;
  }
...
int DLL_CALLCONV FreeImage_GetFIFCount() {
  return (s_plugins != nullptr) ? s_plugins->Size() : 0;
}

FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_GetFIFFromFormat(const char *format) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFormat(format);

    return (node != nullptr) ? (FREE_IMAGE_FORMAT)node->m_id : FIF_UNKNOWN;
  }

  return FIF_UNKNOWN;
}

FREE_IMAGE_FORMAT DLL_CALLCONV FreeImage_GetFIFFromMime(const char *mime) {
  if (s_plugins != nullptr) {
...
const char *DLL_CALLCONV FreeImage_GetFormatFromFIF(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? (node->m_format != nullptr)
                                ? node->m_format
                                : node->m_plugin->format_proc()
                          : nullptr;
  }
...
const char *DLL_CALLCONV FreeImage_GetFIFMimeType(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? (node->m_plugin != nullptr)
                                ? (node->m_plugin->mime_proc != nullptr)
                                      ? node->m_plugin->mime_proc()
                                      : nullptr
                                : nullptr
                          : nullptr;
  }
...
const char *DLL_CALLCONV FreeImage_GetFIFExtensionList(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? (node->m_extension != nullptr)
                                ? node->m_extension
                                : (node->m_plugin->extension_proc != nullptr)
...
const char *DLL_CALLCONV FreeImage_GetFIFDescription(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? (node->m_description != nullptr)
                                ? node->m_description
                                : (node->m_plugin->description_proc != nullptr)
                                      ? node->m_plugin->description_proc()
                                      : nullptr
                          : nullptr;
  }
...
const char *DLL_CALLCONV FreeImage_GetFIFRegExpr(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? (node->m_regexpr != nullptr)
                                ? node->m_regexpr
                                : (node->m_plugin->regexpr_proc != nullptr)
                                      ? node->m_plugin->regexpr_proc()
                                      : nullptr
                          : nullptr;
  }
...
BOOL DLL_CALLCONV FreeImage_FIFSupportsReading(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    ...
    return (node != nullptr) ? node->m_plugin->load_proc != nullptr : FALSE;
  }

  return FALSE;
}

BOOL DLL_CALLCONV FreeImage_FIFSupportsWriting(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? node->m_plugin->save_proc != nullptr : FALSE;
  }

  return FALSE;
}

BOOL DLL_CALLCONV FreeImage_FIFSupportsExportBPP(FREE_IMAGE_FORMAT fif,
                                                 int depth) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr)
               ? (node->m_plugin->supports_export_bpp_proc != nullptr)
                     ? node->m_plugin->supports_export_bpp_proc(depth)
                     : FALSE
               : FALSE;
  }
...
BOOL DLL_CALLCONV FreeImage_FIFSupportsExportType(FREE_IMAGE_FORMAT fif,
                                                  FREE_IMAGE_TYPE type) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr)
               ? (node->m_plugin->supports_export_type_proc != nullptr)
                     ? node->m_plugin->supports_export_type_proc(type)
                     : FALSE
               : FALSE;
  }
...
BOOL DLL_CALLCONV FreeImage_FIFSupportsICCProfiles(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? (node->m_plugin->supports_icc_profiles_proc != nullptr)
...
BOOL DLL_CALLCONV FreeImage_FIFSupportsNoPixels(FREE_IMAGE_FORMAT fif) {
  if (s_plugins != nullptr) {
    PluginNode *node = s_plugins->FindNodeFromFIF(fif);

    return (node != nullptr) ? (node->m_plugin->supports_no_pixels_proc != nullptr)
...
FreeImage_GetFIFFromFilename(const char *filename) {
  if (filename != nullptr) {
    ...
    extension = (place != nullptr) ? ++place : filename;
    ...
    for (int i = 0; i < FreeImage_GetFIFCount(); ++i) {
      ...
      if (s_plugins->FindNodeFromFIF(i)->m_enabled) {
        ...
        if (FreeImage_stricmp(FreeImage_GetFormatFromFIF((FREE_IMAGE_FORMAT)i),
                              extension) == 0) {
          ...
          char *copy = (char *)malloc(
              strlen(FreeImage_GetFIFExtensionList((FREE_IMAGE_FORMAT)i)) + 1);
          memset(copy, 0,
                 strlen(FreeImage_GetFIFExtensionList((FREE_IMAGE_FORMAT)i)) +
                     1);
          memcpy(copy, FreeImage_GetFIFExtensionList((FREE_IMAGE_FORMAT)i),
                 strlen(FreeImage_GetFIFExtensionList((FREE_IMAGE_FORMAT)i)));
...
BOOL DLL_CALLCONV FreeImage_Validate(FREE_IMAGE_FORMAT fif, FreeImageIO *io,
                                     fi_handle handle) {
  if (s_plugins != nullptr) {
    ...
    if (node) {
      ...
      validated = (node != nullptr)
                      ? (node->m_enabled)
                            ? (node->m_plugin->validate_proc != nullptr)
                                  ? node->m_plugin->validate_proc(io, handle)
                                  : FALSE
                            : FALSE
                      : FALSE;
