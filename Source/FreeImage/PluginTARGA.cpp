...
{
public:
	TargaThumbnail() : _w(0), _h(0), _depth(0), _data(nullptr) { 
	}
	~TargaThumbnail() { 
		if(_data) {
			free(_data); 
...
isTARGA20(FreeImageIO *io, fi_handle handle) {
	...
	// read the signature
	if(start_offset + eof - sizeofSig > 0 && start_offset + eof - sizeofSig < eof)
...
template <int nBITS>
inline static void 
_assignPixel(uint8_t* bits, uint8_t* val, BOOL as24bit = FALSE) {
...
template <>
inline void 
_assignPixel<8>(uint8_t* bits, uint8_t* val, BOOL as24bit) {
	*bits = *val;
}

template <>
inline void 
_assignPixel<16>(uint8_t* bits, uint8_t* val, BOOL as24bit) {
...
template <>
inline void 
_assignPixel<24>(uint8_t* bits, uint8_t* val, BOOL as24bit) {
...
template <>
inline void 
_assignPixel<32>(uint8_t* bits, uint8_t* val, BOOL as24bit) {
	if (as24bit) {
