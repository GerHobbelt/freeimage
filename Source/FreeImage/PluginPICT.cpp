...
ReadColorTable( FreeImageIO *io, fi_handle handle, uint16_t* pNumColors, RGBQUAD* pPal ) {
	...
	*pNumColors = numColors;
	...
	for (i = 0; i < numColors; i++) {
		...
		pPal[val].rgbRed = ((uint8_t) (((uint16_t) (Read16( io, handle )) >> 8) & 0xFF));
		pPal[val].rgbGreen = ((uint8_t) (((uint16_t) (Read16( io, handle )) >> 8) & 0xFF));
		pPal[val].rgbBlue = ((uint8_t) (((uint16_t) (Read16( io, handle )) >> 8) & 0xFF));
	}
}
