...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	if (handle && fio) {
		...
		try {
			...
			std::unique_ptr<opj_codec_t, decltype(&opj_destroy_codec)> d_codec(opj_create_decompress(OPJ_CODEC_JP2), &opj_destroy_codec);

			// configure the event callbacks
			// catch events using our callbacks (no local context needed here)
			opj_set_info_handler(d_codec.get(), nullptr, nullptr);
			opj_set_warning_handler(d_codec.get(), jp2_warning_callback, nullptr);
			opj_set_error_handler(d_codec.get(), jp2_error_callback, nullptr);
