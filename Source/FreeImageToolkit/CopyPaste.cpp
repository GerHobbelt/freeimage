...
Combine1(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha) {
	...
	uint8_t *dst_bits = FreeImage_GetBits(dst_dib) + ((size_t)(FreeImage_GetHeight(dst_dib) - FreeImage_GetHeight(src_dib) - y) * FreeImage_GetPitch(dst_dib));
...
Combine4(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha) {
	...
	if (src_pal == nullptr || dst_pal == nullptr) {
		return FALSE;
	}
	...
	uint8_t *dst_bits = FreeImage_GetBits(dst_dib) + ((size_t)(FreeImage_GetHeight(dst_dib) - FreeImage_GetHeight(src_dib) - y) *	FreeImage_GetPitch(dst_dib)) + (x >> 1);
	uint8_t *src_bits = FreeImage_GetBits(src_dib);    
...
Combine8(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha) {
	...
	uint8_t *dst_bits = FreeImage_GetBits(dst_dib) + ((size_t)(FreeImage_GetHeight(dst_dib) - FreeImage_GetHeight(src_dib) - y) * FreeImage_GetPitch(dst_dib)) + (x);
	uint8_t *src_bits = FreeImage_GetBits(src_dib);	
...
Combine16_555(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha) {
	...
	uint8_t *dst_bits = FreeImage_GetBits(dst_dib) + ((size_t)(FreeImage_GetHeight(dst_dib) - FreeImage_GetHeight(src_dib) - y) * FreeImage_GetPitch(dst_dib)) + (x * 2);
	uint8_t *src_bits = FreeImage_GetBits(src_dib);	
...
Combine24(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha) {
	...
	uint8_t *dst_bits = FreeImage_GetBits(dst_dib) + ((size_t)(FreeImage_GetHeight(dst_dib) - FreeImage_GetHeight(src_dib) - y) * FreeImage_GetPitch(dst_dib)) + (x * 3);
	uint8_t *src_bits = FreeImage_GetBits(src_dib);	
...
Combine32(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y, unsigned alpha) {
	...
	uint8_t *dst_bits = FreeImage_GetBits(dst_dib) + ((size_t)(FreeImage_GetHeight(dst_dib) - FreeImage_GetHeight(src_dib) - y) * FreeImage_GetPitch(dst_dib)) + (x * 4);
	uint8_t *src_bits = FreeImage_GetBits(src_dib);	
...
CombineSameType(FIBITMAP *dst_dib, FIBITMAP *src_dib, unsigned x, unsigned y) {
	...
	uint8_t *dst_bits = FreeImage_GetBits(dst_dib) + ((size_t)(dst_height - src_height - y) * dst_pitch) + (x * (src_line / src_width));
	uint8_t *src_bits = FreeImage_GetBits(src_dib);	
...
@return Returns the subimage if successful, nullptr otherwise.
*/
...
FreeImage_Copy(FIBITMAP *src, int left, int top, int right, int bottom) {
	...
	int64_t src_width = FreeImage_GetWidth(src);
	...
	int64_t dst_width = (right - left);
	int64_t dst_height = (bottom - top);
	...
	// get the dimensions
	int64_t dst_line = FreeImage_GetLine(dst);
	int64_t dst_pitch = FreeImage_GetPitch(dst);
	int64_t src_pitch = FreeImage_GetPitch(src);
...
/** @brief Creates a dynamic read/write view into a FreeImage bitmap.
 ...
 @return Returns a handle to the newly created view or nullptr if the view
 was not created.
 */
FIBITMAP * DLL_CALLCONV
