	...
	for(int y = 0; y < height; y++) {
		...
		for (int x = 0; x < width; x++, bits += 4) {
			...
			if(alpha == 0x00) {
				...
				bits[FI_RGBA_BLUE] = (uint8_t)((alpha * (uint16_t)bits[FI_RGBA_BLUE]) / 255);
				bits[FI_RGBA_GREEN] = (uint8_t)((alpha * (uint16_t)bits[FI_RGBA_GREEN]) / 255);
				bits[FI_RGBA_RED] = (uint8_t)((alpha * (uint16_t)bits[FI_RGBA_RED]) / 255);
			}
		}
	}
	return TRUE;
