...
PluginList::AddNode(FI_InitProc init_proc, void *instance, const char *format, const char *description, const char *extension, const char *regexpr) {
	if (init_proc != nullptr) {
		...
		if (the_format != nullptr) {
			if (FindNodeFromFormat(the_format) == nullptr) {
	...
	if (s_plugin_reference_count++ == 0) {
		...
		if (s_plugins) {
			...
			*/
			...
			s_plugins->AddNode(InitMNG, nullptr, "JNG", "JPEG Network Graphics", "jng", "");
			...
			if (!load_local_plugins_only) {
				...
				if (GetCurrentDirectory(2 * _MAX_PATH, current_dir) != 0) {
					if (GetModuleFileName(nullptr, module, 2 * _MAX_PATH) != 0) {
...
FreeImage_GetFIFFromFormat(const char *format) {
	if (s_plugins != nullptr) {
		...
		return (node != nullptr) ? (node->m_enabled) ? (FREE_IMAGE_FORMAT)node->m_id : FIF_UNKNOWN : FIF_UNKNOWN;
	}
